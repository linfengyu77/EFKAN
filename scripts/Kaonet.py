...
class FNO2d(nn.Module):
    def __init__(self, modes1, modes2,  width,n_out,layer_num=4,last_size=128,act_func="gelu"):
        ...
        for _ in range(layer_num):
            ...
            # self.conv.append(KAN_Convolutional_Layer(
            # n_convs = 1,
            # kernel_size= (1, 1),
            # device=device))
        ...
        # self.kan = KAN([64*64, 10, 10, n_out])
    ...
    def forward(self, x):
        ...
        x = self.activation(x)
        x = self.fc2(x)
        # x = self.kan(x.view(-1, x.shape[-2]*x.shape[-3]))
...
class FNN(nn.Module):
    def __init__(self, layer_sizes, act_func, init_func):
        ...
        self.kan = KAN([layer_sizes[0], 20, layer_sizes[-1]])
