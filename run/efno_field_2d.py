"""
predict electric and magnetic field

usage: python efno_2d.py field_best
"""
...
def get_batch_data(TRAIN_PATH, TEST_PATH, ntrain, ntest, r_train,s_train,r_test,s_test, batch_size,n_out):
    ...
    ----------
        ...
        - n_out      : number of output channels, here is 3: Ex, Ey, Hy
    '''
    print("begin to read data")
    key_map0 = ['Ex','Hy','Ey','Hx']
    ...
    data_train = [reader.read_complex(key_map[i]) for i in range(len(key_map))]
    y_train = torch.cat([torch.stack([torch.log10(torch.abs(data_train[i])), torch.atan2(data_train[i].imag,data_train[i].real)], axis=-1)\
        for i in range(len(data_train))],axis =-1)[:ntrain,::r_train[2],::r_train[3],:][:,:s_train[2],:s_train[3],:]
    ...
    data_test = [reader_test.read_complex(key_map[i]) for i in range(len(key_map))]
    y_test = torch.cat([torch.stack([torch.log10(torch.abs(data_test[i])), torch.atan2(data_test[i].imag,data_test[i].real)], axis=-1)\
        for i in range(len(key_map))],axis =-1)[:ntest,::r_test[2],::r_test[3],:][:,:s_test[2],:s_test[3],:]
...
def run_train(model,batch_size, s_train,s_test,loc_train,loc_test,train_loader, test_loader,
              y_normalizer,loss_train,loss_test, optimizer, scheduler,epochs, 
    ...
    for ep in range(epochs):
        t1 = default_timer()
        model.train()
        train_l2 = batch_train(model, batch_size,s_train ,loc_train,train_loader, y_normalizer,  loss_train, optimizer, scheduler, device)
        model.eval()
        test_l2 = batch_validate(model, batch_size,s_test,loc_test,test_loader, y_normalizer, loss_test, device)
        train_l2/= ntrain
        test_l2 /= ntest
...
def main(item):
    ...
    with open( 'config_field.yml') as f:
    ...
    loss_train = LpLoss(p=2,size_average=False)
    loss_test  = LpLoss(p=2,size_average=False)

    log_file = open(log_path,'a+')
    print("####################")
    print("begin to train model")
    run_train(model, batch_size,s_train,s_test,loc_train,loc_test,train_loader, test_loader, y_normalizer, loss_train,loss_test, optimizer, scheduler, epochs, \
