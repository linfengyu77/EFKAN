...
usage: nohup python efno_mlp.py 'random_smooth_mlpkan_layer6_super' > smooth_mlpkan_layer6_super.out 2>&1 &
"""
...
def get_batch_data(TRAIN_PATH, TEST_PATH, ntrain, ntest, r_train,s_train,r_test,s_test, batch_size,n_out):
    ...
    print(f"Data Loading with {t_read1-t_read0:.3f} s")
...
def main(configs, item):
    ...
    tn = default_timer()
    print(f'Training Time:{tn-t0:.3f}s')
    print(f'# Training Time:{tn-t0:.3f}s',file=log_file)
    log_file.close()
